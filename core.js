(function () {
    ...
    function showBlockedMessage(customMsg) {
        ...
        var msgText = customMsg || "تم ايقاف الـ Tool";
    ...
    window.adelServerConfig = window.adelServerConfig || { isActive: true, pass: "02026", hols: [], blockedDevices: [], accDb: {} };
        ...
        fetch(dbUrl).then(function (r) { return r.json(); }).then(function (data) {
            if (data !== null && !data.error) {
                if (typeof data === 'boolean') window.adelServerConfig.isActive = data;
                    ...
                    if (data.accDb) window.adelServerConfig.accDb = data.accDb;
                    else window.adelServerConfig.accDb = {};
                }
            }
    ...
    function runAdelEngine() {
        ...
        try {
            ...
            // =========================================================================
            // القسم الخاص بتصدير البيانات (Custom Export Modal)
            // =========================================================================
            window.exportCSV = function () {
                var currentDb = [];
                try { currentDb = JSON.parse(localStorage.getItem("adel_db") || "[]"); } catch (e) { }
                
                if (currentDb.length === 0) {
                    alert("لا يوجد أي بيانات مسجلة في النظام لتصديرها!");
                    return;
                }

                if (document.getElementById('adel-export-modal')) return;

                var modal = document.createElement('div');
                modal.id = 'adel-export-modal';
                modal.dir = "rtl";
                modal.style.cssText = "position:fixed;top:50%;left:50%;transform:translate(-50%, -50%);background:rgba(15,23,42,0.98);border:1px solid rgba(56,189,248,0.3);border-radius:12px;padding:20px 25px;color:#f1f5f9;font-family:'Segoe UI',sans-serif;z-index:2147483647;box-shadow:0 20px 60px rgba(0,0,0,0.9);min-width:280px;backdrop-filter:blur(20px);animation:adelPopIn 0.3s ease-out;display:flex;flex-direction:column;gap:15px;";

                var header = "<div style='text-align:center;font-size:15px;font-weight:800;color:#0ea5e9;border-bottom:1px solid rgba(255,255,255,0.1);padding-bottom:10px;margin-bottom:5px;letter-spacing:1px;'>تصدير البيانات</div>";
                var label = "<div style='font-size:12px;color:#94a3b8;margin-bottom:-8px;font-weight:bold;'>اختر التاريخ:</div>";
                
                var todayKey = new Date().toLocaleDateString('en-CA');
                var inputHtml = "<input type='date' id='adel-export-date' value='" + todayKey + "' style='width:100%;padding:10px 12px;background:rgba(15,23,42,0.8);border:1px solid rgba(56,189,248,0.4);border-radius:8px;color:#fff;font-size:14px;font-weight:bold;outline:none;cursor:pointer;font-family:monospace;color-scheme:dark;'>";

                var buttons = "<div style='display:flex;gap:10px;margin-top:5px;'>" +
                    "<button id='adel-btn-export-date' style='flex:1;background:linear-gradient(135deg,#0ea5e9,#0284c7);color:#fff;border:none;padding:10px;border-radius:8px;cursor:pointer;font-weight:bold;font-size:13px;transition:0.2s;letter-spacing:1px;text-transform:uppercase;'>Export</button>" +
                    "<button id='adel-btn-cancel' style='flex:1;background:rgba(244,63,94,0.1);color:#f43f5e;border:1px solid rgba(244,63,94,0.3);padding:10px;border-radius:8px;cursor:pointer;font-weight:bold;font-size:13px;transition:0.2s;letter-spacing:1px;text-transform:uppercase;'>Cancel</button>" +
                    "</div>";

                modal.innerHTML = header + label + inputHtml + buttons;
                document.body.appendChild(modal);

                document.getElementById('adel-btn-export-date').onmouseover = function(){ this.style.transform = 'translateY(-2px)'; };
                document.getElementById('adel-btn-export-date').onmouseout = function(){ this.style.transform = 'translateY(0)'; };
                document.getElementById('adel-btn-cancel').onmouseover = function(){ this.style.background = '#f43f5e'; this.style.color = '#fff'; };
                document.getElementById('adel-btn-cancel').onmouseout = function(){ this.style.background = 'rgba(244,63,94,0.1)'; this.style.color = '#f43f5e'; };

                document.getElementById('adel-btn-cancel').onclick = function() {
                    modal.remove();
                };

                document.getElementById('adel-btn-export-date').onclick = function() {
                    var userDate = document.getElementById('adel-export-date').value;
                    modal.remove(); 

                    var filteredDb = currentDb.filter(function(row) {
                        return row.key === userDate;
                    });

                    if (filteredDb.length === 0) {
                        alert("لا يوجد بيانات لتصديرها في التاريخ المختار: " + userDate);
                        return;
                    }

                    var csvContent = "\uFEFF";
                    csvContent += "AWB,Status,Type/Company,Product,Details,Date,Time\n";

                    filteredDb.forEach(function(row) {
                        var a = '"' + (row.awb || '') + '"';
                        var s = '"' + (row.drs || '') + '"';
                        var c = '"' + (row.co || '') + '"';
                        var p = '"' + (row.prod || '') + '"';
                        ...
                        csvContent += [a, s, c, p, r, ud, ut].join(",") + "\n";
                    });
                    ...
                    link.setAttribute("download", "SkyBag_Report_" + userDate + ".csv");
            ...
            setInterval(function () {
                ...
                try {
                    ...
                    var accDb = window.adelServerConfig.accDb || {};
                    
                    if (Object.keys(accDb).length > 0 && window.lastSeenAWB) {
                        try {
