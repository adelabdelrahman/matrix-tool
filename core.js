(function () {
    ...
    function runAdelEngine() {
        ...
        try {
            ...
            // =========================================================================
            // القسم الثالث: التصميم والواجهة (CSS)
            // =========================================================================
            ...
            window.loadNext = function () {
                ...
                if (awb) {
                    ...
                    awb.focus();
                    setTimeout(function () {
                        var e = new KeyboardEvent('keydown', { bubbles: true, cancelable: true, key: 'Enter', keyCode: 13 });
                        awb.dispatchEvent(e);
                    }, 50);
                }
            };
            ...
            var rec = function (d) {
                if (!d.a) return;
                var n = new Date();
                var k = n.toLocaleDateString('en-CA');
                var idx = -1;
                for (var i = 0; i < db.length; i++) {
                    if (db[i].awb === d.a && db[i].key === k) {
                        idx = i;
                        break;
                    }
                }
                var u = n.toLocaleDateString('en-GB');
                var ut = n.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true });
            ...
            window.drawPanel = function () {
                ...
                document.getElementById('pass-inp').onkeydown = function (e) {
                ...
                var upd = function (i) {
                    navs.forEach(function (x, n) {
                        if (n === i) x.classList.add('active');
                ...
                window.setM = function (m, s) {
                    ...
                    if (m === "bank" && !window.bankDate) {
                        var t = new Date();
                        var def = t.getDate() + "/" + (t.getMonth() + 1);
                        var pr = prompt("Date:", def);
                        if (pr) window.bankDate = pr;
                    }
                    var t = document.getElementById("ContentPlaceHolder1_txt_AWB_I");
                    if (t) t.focus();
                };

                navs = Array.from(document.querySelectorAll('#mode-list .mode-btn'));
                upd(0);
                ...
                d.onkeydown = function (e) {
                    ...
                    if (e.key === "ArrowDown") {
                        ...
                        e.preventDefault();
                        c = (c - 1 + navs.length) % navs.length;
                        upd(c);
                    } else if (e.key === "Enter") {
                        e.preventDefault();
                        if (navs[c]) navs[c].click();
                    }
                };
            };
            ...
            var updatePick = function () {
                if (!ps || !ps.o) return;
                ps.o.forEach(function (b, i) {
                    if (i === ps.i) b.classList.add('sel');
            ...
            var showPicker = function (n, c, cb) {
                ...
                d.id = "adel-picker";
                d.tabIndex = "-1";
                d.innerHTML = "<div style='color:#0ea5e9;font-weight:900;margin-bottom:15px;font-size:12px;letter-spacing:2px'>SELECT NAME</div>";
                var opts = [];
                ...
                document.body.appendChild(d);
                ps = { i: 0, o: opts, n: n, c: c, cb: cb, el: d };
                updatePick();
            };
            ...
            document.addEventListener("keydown", function (e) {
                ...
                if (e.key === "Alt") {
                    var el = document.activeElement;
                    if (el && el.id.indexOf("txt_AWB") > -1) return;
                    e.preventDefault();
                    try {
                        var m = window.adelMode;
                        var get = function (id) { return ASPx.GetControlCollection().Get("ContentPlaceHolder1_ASPxGridView1_DXEFL_" + id); };
                        var tds = document.getElementsByTagName("td"), raw = "";
                        for (var i = 0; i < tds.length; i++) {
                            if (tds[i].innerText.trim() === "Receiver Name") {
                                var r = tds[i].parentElement.nextElementSibling;
                                if (r) raw = r.cells[tds[i].cellIndex].innerText.trim();
                                break;
                            }
                        }
                        var cd = get("DXEditor6"), cs = get("DXEditor4"), cn = get("DXEditor2");
                        if (m === "man") {
                            var d = new Date(), n = new Date();
                            n.setHours(0, 0, 0, 0);
                            for (var i = 0; i < tds.length; i++) {
                                if (tds[i].innerText.trim() === "Bill Date") {
                                    var r = tds[i].parentElement.nextElementSibling;
                                    if (r) {
                                        var t = r.cells[tds[i].cellIndex].innerText.trim();
                                        if (t.indexOf("/") > -1) {
                                            var s = t.split("/");
                                            var bdt = new Date(s[2], s[0] - 1, s[1]);
                                            var ld = new Date(bdt);
                            ...
                            var next = function () {
                                ...
                                if (cr) {
                                    cr.SetFocus();
                                    if (cr.ShowDropDown) cr.ShowDropDown();
                                    cr.SelectedIndexChanged.ClearHandlers();
                                    cr.SelectedIndexChanged.AddHandler(function () {
                                        setTimeout(function () {
                                            var tid = (m === "bank") ? "DXEditor8" : "DXEditor3";
                                            var el = document.getElementById("ContentPlaceHolder1_ASPxGridView1_DXEFL_" + tid + "_I");
                                            if (el) {
                                                el.focus();
                                                el.select();
                                                if (m !== "bank") {
                                                    el.onkeyup = function () {
                                                        var v = el.value, ok = (v.length === 14 && (v.startsWith("2") || v.startsWith("3")));
                            ...
                            if (m === "bank") {
                                if (!window.bankDate) {
                                    var p = prompt("Date?");
                                    if (p) window.bankDate = p;
                                }
                                if (window.bankDate) {
                                    var p = window.bankDate.split("/");
                                    var d = new Date();
                                    d.setHours(0, 0, 0, 0);
                                    d.setDate(parseInt(p[0]));
                                    d.setMonth(parseInt(p[1]) - 1);
                                    if (p[2]) d.setFullYear(parseInt(p[2]));
                                    d = checkDate(d, m);
                                    if (cd) cd.SetDate(d);
                                }
                                if (cs) cs.SetValue(6);
                                if (cn) cn.SetValue("Bank Stamp");
                                if (cn) cn.SetFocus();
                                next();
                            } else {
                                var n = new Date();
                                ...
                                for (var i = 0; i < tds.length; i++) {
                                    if (tds[i].innerText.trim() === "Bill Date") {
                                        var r = tds[i].parentElement.nextElementSibling;
                                        if (r) {
                                            var t = r.cells[tds[i].cellIndex].innerText.trim().split("/");
                                            if (t.length > 1) {
                                                billDate = new Date(t[2], t[0] - 1, t[1]);
                                                found = true;
                                            }
                                        }
                                        break;
                                ...
                                if (found) {
                                    ...
                                    if (target >= n) target = y;
                                    target = checkDate(target, m);
                                    if (cd) cd.SetDate(target);
                                }
                                if (cs) cs.SetValue(6);
                                if (cn) {
                                    cn.SetFocus();
                                    if (/[a-zA-Z]/.test(raw)) raw = raw.replace(/[\u0600-\u06FF]/g, '').replace(/\s+/g, ' ').trim();
                                    var p = raw.split(/[:;,\-|\/\*&+]|\bOR\b|\bAND\b/i).map(x => x.replace(/[\u0600-\u06FF]/g, '').replace(/\s+/g, ' ').trim()).filter(x => x.length > 1);
                                    var he = p.some(n => /[a-zA-Z]/.test(n));
                                    if (he) p = p.filter(n => /[a-zA-Z]/.test(n));
                                    if (p.length > 1) showPicker(p, cn, next);
                                    else {
                                        cn.SetValue(p[0] || raw);
                                        next();
                                    }
            ...
            setInterval(function () {
                ...
                try {
                    ...
                    var st = document.getElementById("ContentPlaceHolder1_ASPxGridView1_DXEFL_DXEditor4_I");
                    var dt = document.getElementById("ContentPlaceHolder1_ASPxGridView1_DXEFL_DXEditor6_I");
                            ...
                            for (var i = 0; i < tds.length; i++) {
                                ...
                                if (txt.indexOf("Acc") > -1 || txt.indexOf("No") > -1) {
                                    ...
                                    if (!num) {
                                        var r = tds[i].parentElement.nextElementSibling;
                                        if (r) {
                                            var belowCell = r.cells[tds[i].cellIndex];
                                            if (belowCell) {
                                                var txt2 = belowCell.innerText.trim();
                                                var m2 = txt2.match(/\d{4,}/);
                                                if (m2) num = m2[0];
                                            }
                                        }
                                    }
                                    if (num && accDb[num]) {
                                        window.adelSub = accDb[num].n;
                                        window.adelProd = accDb[num].p;
                                        break;
                                    }
                                }
                            }
                    ...
                    if (doEdit) {
                        var b = document.getElementById("ContentPlaceHolder1_ASPxGridView1_DXCBtn0_I");
                    ...
                    if (pm && pm.IsVisible()) {
                        var t = "";
                        try { t = pm.GetMainElement().innerText.toUpperCase(); } catch (x) { }
                        pm.Hide();
                        
                        if (t.indexOf("ERR") === -1 && t.indexOf("FAIL") === -1 && t.indexOf("خطأ") === -1) {
                            if (window.lastSeenAWB) {
                                var finalSt = window.lastValidSt || "Unknown";
                                finalSt = finalSt.replace(/^[;\s]+/, "").trim();
